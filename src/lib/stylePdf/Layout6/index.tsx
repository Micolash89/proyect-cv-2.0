  ...
  return (
    <Document title={`CV - ${user.fullName}`}>
      <Page size="A4" style={styles.page}>
        <View style={styles.header}><Text style={styles.name}>{name}</Text><View style={styles.contactInfo}>{user.phone&&<Text>{user.phone}</Text>}{user.email&&<Text>{user.email}</Text>}{user.location&&<Text>{user.location}</Text>}</View></View>
