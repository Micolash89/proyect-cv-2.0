...
const parseDate = (d: string | undefined) => {
  if (!d) return "";
  try { const da = new Date(d); return `${da.getMonth()+1}/${da.getFullYear()}`; } 
  catch { return d; }
};
...
export const Layout4: React.FC<{ user: UserCV; options?: Partial<OptionsPDF> }> = ({ user, options }) => {
  ...
  const exp = user.experience.map((e,i) => <View key={i} style={styles.entryContainer}><View style={styles.entryHeader}><Text style={styles.institution}>{e.company}</Text><Text style={styles.dates}>{parseDate(e.startDate)} - {e.current?"Actualidad":parseDate(e.endDate)}</Text></View><Text style={styles.degree}>{e.position}</Text></View>);
    ...
    <Document title={`CV - ${user.fullName}`}>
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          ...
          <View style={styles.contactInfo}>{user.phone && <Text>{user.phone}</Text>}{user.email && <Text>{user.email}</Text>}{user.location && <Text>{user.location}</Text>}</View>
        </View>
