...
export default function AdminNewCVPage() {
  ...
  const updateFormData = useCallback(
    (data: Partial<typeof formData>, touchedPath?: string) => {
      setFormData((prev) => {
  ...
  const updateLanguage = (id: string, field: string, value: string) => {
    const index = formData.languages.findIndex(
  ...
  const handleDataExtracted = useCallback((data: ExtractedCVData) => {
    ...
    setFormData((prev) => ({
      ...
      fullName: buildFullName(
        splitName.name || prev.name,
        splitName.lastName || prev.lastName,
      ),
      ...
      movilidad:
        typeof data.movilidad === "boolean" ? data.movilidad : prev.movilidad,
      incorporacionInmediata:
        typeof data.incorporacionInmediata === "boolean"
          ? data.incorporacionInmediata
          : prev.incorporacionInmediata,
      office: typeof data.office === "boolean" ? data.office : prev.office,
      disponibilidad:
        data.disponibilidad === "partTime"
          ? "partTime"
                  ...
                  <div className="grid gap-3 sm:grid-cols-4">
                    ...
                    <label className="flex items-center gap-2 rounded-md border bg-background px-3 py-2">
                      <input
                        ...
                        onChange={(e) =>
                          updateFormData({ office: e.target.checked }, "office")
                        }
                      />
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4 text-sm">
                    <div className="grid gap-3 sm:grid-cols-2">
                      <div className="rounded-md border bg-muted/30 p-3">
                        ...
                        <p className="mt-1 font-medium">
                          {buildFullName(formData.name, formData.lastName) ||
                            "No especificado"}
                        </p>
                      </div>
                      <div className="rounded-md border bg-muted/30 p-3">
                      ...
                      <div className="rounded-md border bg-muted/30 p-3">
                        ...
                        <p className="mt-1 font-medium">
                          {templateOptions.find(
                            (template) => template.id === formData.selectedTemplate,
                    ...
                    <ExpandableSectionCard
                      title={`Experiencia (${formData.experience.length})`}
                      icon={<Briefcase className="h-5 w-5" />}
                      summary={
                        formData.experience.length > 0
                          ? `${formData.experience[0].company || "Sin empresa"} · ${formData.experience[0].position || "Sin puesto"}`
                          : "Sin experiencias agregadas"
                      }
                      open={expandedReviewSections.experience}
                      onToggle={() => toggleReviewSection("experience")}
                        ...
                        formData.experience.map((exp, index) => (
                          ...
                          >
                            ...
                            <p className="mt-1 text-xs text-muted-foreground">
                              {exp.startDate || "Sin fecha"} - {exp.current ? "Actualidad" : exp.endDate || "Sin fecha"}
                            </p>
                          </div>
                        ))
                      ) : (
                        <p className="text-sm text-muted-foreground">
                          Aún no agregaste experiencia.
                        </p>
                      )}
                    </ExpandableSectionCard>

                    <ExpandableSectionCard
                      title={`Educación (${formData.education.length})`}
                      icon={<GraduationCap className="h-5 w-5" />}
                      summary={
                        formData.education.length > 0
                          ? `${formData.education[0].institution || "Sin institución"} · ${formData.education[0].degree || "Sin título"}`
                          : "Sin educación agregada"
                      }
                      open={expandedReviewSections.education}
                      onToggle={() => toggleReviewSection("education")}
                      className="border-border/70"
                      contentClassName="space-y-3"
                    >
                      {formData.education.length > 0 ? (
                        formData.education.map((edu, index) => (
                          ...
                          >
                            ...
                            <p className="mt-1 text-xs text-muted-foreground">
                              {edu.startDate || "Sin fecha"} - {edu.endDate || "Actualidad"}
                            </p>
                          </div>
                        ))
                      ) : (
                        <p className="text-sm text-muted-foreground">
                          Aún no agregaste educación.
                        </p>
                      )}
                    </ExpandableSectionCard>

                    <ExpandableSectionCard
                      title={`Cursos y certificaciones (${formData.certifications.length})`}
                      icon={<BookOpen className="h-5 w-5" />}
                      summary={
                        formData.certifications.length > 0
                          ? `${formData.certifications[0].title || "Sin título"} · ${formData.certifications[0].institution || "Sin institución"}`
                          : "Sin cursos agregados"
                      }
                      open={expandedReviewSections.certifications}
                      onToggle={() => toggleReviewSection("certifications")}
                      className="border-border/70"
                      contentClassName="space-y-3"
                    >
                      {formData.certifications.length > 0 ? (
                        formData.certifications.map((course, index) => (
                          <div
                            ...
                            className="rounded-md border bg-background p-3 text-sm"
                          >
                            <p className="font-medium">Curso {index + 1}</p>
                            <p className="text-muted-foreground">
                              {course.title || "Título no especificado"}
                            </p>
                            <p className="text-xs text-muted-foreground">
                              {course.institution || "Sin institución"}
                            </p>
                          </div>
                        ))
                      ) : (
                    ...
                    <ExpandableSectionCard
                      title={`Idiomas (${formData.languages.length})`}
                      icon={<Globe className="h-5 w-5" />}
                      summary={
                        formData.languages.length > 0
                          ? `${formData.languages[0].language || "Sin idioma"} · ${formData.languages[0].level || "Sin nivel"}`
                          : "Sin idiomas agregados"
                      }
                      open={expandedReviewSections.languages}
                      onToggle={() => toggleReviewSection("languages")}
                      className="border-border/70"
                      contentClassName="space-y-3"
                    >
                      {formData.languages.length > 0 ? (
                        formData.languages.map((lang, index) => (
                          <div
                            ...
                            className="rounded-md border bg-background p-3 text-sm"
                          >
                            <p className="font-medium">Idioma {index + 1}</p>
                            <p className="text-muted-foreground">
                              {lang.language || "Idioma no especificado"} · {lang.level || "Nivel no especificado"}
                            </p>
                          </div>
                        ))
                      ) : (
                    ...
                    <ExpandableSectionCard
                      title={`Habilidades (${formData.skills.length})`}
                      ...
                      open={expandedReviewSections.skills ?? false}
                      onToggle={() => toggleReviewSection("skills")}
                      className="border-border/70"
                      contentClassName="space-y-3"
                    >
                      {formData.skills.length > 0 ? (
